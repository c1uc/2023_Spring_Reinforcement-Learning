...
class Critic(nn.Module):
    def __init__(self, hidden_size, num_inputs, action_space):
        ...
        self.input_s = nn.Linear(num_inputs, hidden_size)
        self.input_a = nn.Linear(num_outputs, hidden_size)
        self.fc = nn.Linear(hidden_size, hidden_size)
    ...
    def forward(self, inputs, actions):
        ...
        x = F.relu(self.input_s(inputs))
        q = F.relu(self.input_a(actions))
        x = F.relu(self.fc(x + q))
        x = self.output(x)
...
class DDPG(object):
    ...
    def update_parameters(self, batch):
        ...
        q_target = Variable(self.gamma * self.critic_target(next_state_batch, self.actor_target(next_state_batch)).squeeze().detach())
        q_target = reward_batch + q_target
