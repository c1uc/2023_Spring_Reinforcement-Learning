...
class TRPO(object):
    ...
    def update_agent(self, update_step, delta=0.01, backtrack_ratio=0.8, max_backtracks=15, damping=1e-1):
        ...
        for tr in self.memory:
            R_plus = 0
            tr_returns_plus = []
            rewards_plus = tr.reward + self.simhash.get_bonus(tr.key)
            for reward_plus in rewards_plus[::-1]:
                R_plus = reward_plus + self.gamma * R_plus
                tr_returns_plus.append(R_plus)
            tr_returns_plus = torch.as_tensor(tr_returns_plus[::-1]).unsqueeze(1)
            returns_plus.append(tr_returns_plus)
        returns_plus = torch.cat(returns_plus, dim=0).float()
        returns_plus = (returns_plus - returns_plus.mean()) / (returns_plus.std() + 1e-8)
        ...
        returns_plus = torch.from_numpy(returns_plus.numpy()[idx])
        ...
        advantages = (returns_plus - baselines).detach().flatten()
...
if __name__ == '__main__':
    ...
    agent = TRPO(train_env, test_env, gamma=0.995, lr_c=3e-4, beta=0.01)
