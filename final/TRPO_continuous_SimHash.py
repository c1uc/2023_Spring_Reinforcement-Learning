...
writer = SummaryWriter("./tb_record_continuous_simhash")
...
class Critic(nn.Module):
    ...
    def forward(self, x):
        x = torch.as_tensor(x, dtype=torch.float32)
        x = torch.relu(torch.tanh(self.affine1(x)))
        # x = torch.tanh(self.affine2(x))
...
class TRPO:
    def __init__(self, env, k=32, log_interval=1, eval_interval=50,
                 gamma=0.99, tau=1, l2_reg=1e-3, max_kl=1e-2, damping=1e-1, batch_size=5000):
        self.env = env
        ...
        for i_episode in range(1, num_epoch+1):
            ...
            while num_steps < self.batch_size:
                ...
                for t in range(10000):  # Don't infinite loop while learning
                    ...
                    # reward = 1 if (done and t < 199) else 0
                    reward_sum += reward
...
if __name__ == '__main__':
    ...
    agent = TRPO(env, eval_interval=999, batch_size=50000)
    agent.train()
