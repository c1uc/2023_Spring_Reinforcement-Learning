...
def value_iteration(env, gamma=0.9, max_iterations=10**6, eps=10**-3):
    ...
    for _ in range(max_iterations):
        ...
        for s in range(num_spaces):
            best = -np.inf
            for a in range(num_actions):
                Q = np.dot(P[s, a], R[s, a] + gamma * Vs)
                best = max(best, Q)
            delta = max(delta, np.abs(best - Vs[s]))
            Vs[s] = best
        if delta < eps:
            break
...
def policy_iteration(env, gamma=0.9, max_iterations=10**6, eps=10**-3):
    """ 
        ...
        ----------
            ...
            gamma: float
                the discount factor for rewards
            max_iterations: int
                maximum number of iterations for the policy evaluation in policy iteration
            eps: float
    ...
    for _ in range(max_iterations):
        ...
        while True:
            ...
            for s in range(num_spaces):
                a = policy[s]
                Q = np.dot(P[s, int(a)], R[s, int(a)] + gamma * Vs)
                delta = max(delta, np.abs(Q - Vs[s]))
                Vs[s] = Q
            if delta < eps:
                break

        n_policy = np.zeros(num_spaces)
        for s in range(num_spaces):
            Qs = np.zeros(num_actions)
            for a in range(num_actions):
                Q = np.dot(P[s, a], R[s, a] + gamma * Vs)
                Qs[a] = Q
            n_policy[s] = np.argmax(Qs)

        if np.all(n_policy == policy):
            break
