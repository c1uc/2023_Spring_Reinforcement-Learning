...
class Policy(nn.Module):
    ...
    def __init__(self):
        ...
        torch.nn.init.xavier_normal_(self.output_policy[1].weight)
        torch.nn.init.xavier_normal_(self.output_value[0].weight)
        torch.nn.init.xavier_normal_(self.output_value[1].weight)
    ...
    def forward(self, state):
        ...
        state_value = self.output_value(x)
    ...
    def calculate_loss(self, gamma=0.999):
        ...
        values = torch.tensor([s.value for s in saved_actions])
        advantages = GAE(gamma, 0.99, None)(self.rewards, values, [])
        ...
        for a, s in zip(advantages, saved_actions):
            policy_losses.append(-g * s.log_prob * a)
            g = g * gamma
...
class GAE:
    ...
    def __call__(self, rewards, values, done):
        ...
        for r, v in zip(reversed(rewards), reversed(values)):
            td_err = r + next_v * self.gamma - v
            advantage = td_err + advantage * self.lambda_ * self.gamma
            next_v = v
